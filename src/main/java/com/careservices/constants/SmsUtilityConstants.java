/**
 * 
 */
package com.careservices.constants;

import java.util.Collection;

/**
 * @author JARVIS
 *
 */
public class SmsUtilityConstants {

	public static final String GroupNameRequired = "Group Name Is Required";
	public static final String NameCanNotBeNull = "Conatct Name Required";
	public static final String MobileCanNotBeNull = "Mobile Number Can Not Be Null";
	public static final String DataIsNull = "Your Data Is Empty";
	public static final String ContactIdNotDefine = "Contact Id Can Not Be Null";
	

}
